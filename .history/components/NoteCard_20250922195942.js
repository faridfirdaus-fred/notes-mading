...
export default function NoteCard({ note }) {
  ...
  return (
    ...
    >
      {/* Plester (Tape) di bagian atas */}
      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/4 z-10">
        <div className="w-6 h-2 bg-gray-300 opacity-90 rounded-sm transform rotate-0 shadow-sm"></div>
      </div>
      
      {/* Paper texture effect */}
      <div className="relative bg-opacity-90 pt-5 pb-3 px-3">
        <div className="relative h-52 w-52 mx-auto mb-3 border-2 border-gray-100 shadow-sm">
          <Image
            src={note.imageUrl}
            alt={note.name}
            fill
