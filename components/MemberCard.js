...
const memberStyles = {
  ...
  'Farid': { color: 'bg-green-50', rotation: -3 },
  'Rai': { color: 'bg-orange-50', rotation: 2 },
  'default': { color: 'bg-gray-50', rotation: 0 }
};
...
export default function MemberCard({ member }) {
  ...
  return (
    ...
    >
      ...
      {member.name === 'Farid' && (
        <>
          <div className="absolute top-18 -left-3 z-20 w-12 h-6 bg-green-200/80 rounded-sm shadow-md transform -rotate-15"></div>
          <div className="absolute top-34 -right-3 z-20 w-10 h-6 bg-gray-300/80 rounded-sm shadow-md transform rotate-12"></div>
        </>
      )}
      {member.name === 'Rai' && (
        <>
          <div className="absolute top-20 -left-3 z-20 w-14 h-6 bg-orange-200/80 rounded-sm shadow-md transform -rotate-8"></div>
          <div className="absolute top-38 -right-3 z-20 w-12 h-6 bg-gray-300/80 rounded-sm shadow-md transform rotate-18"></div>
        </>
      )}
      ...
      <div className="relative bg-opacity-90 pt-3 pb-3 px-3 overflow-hidden">
        ...
        <div className="px-2">
          <h3 className={`text-lg font-bold ${
            ...
            member.name === 'Talitha' ? 'text-purple-800' :
            member.name === 'Farid' ? 'text-green-800' :
            member.name === 'Rai' ? 'text-orange-800' :
            'text-gray-800'
          ...
          {member.role && (
            <div className={`mt-2 ${
              ...
              member.name === 'Talitha' ? 'bg-purple-100' :
              member.name === 'Farid' ? 'bg-green-100' :
              member.name === 'Rai' ? 'bg-orange-100' :
              'bg-gray-100'
